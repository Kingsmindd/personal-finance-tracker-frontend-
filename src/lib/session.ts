...
export interface SessionUser { id: string; email: string; name?: string; currency: string }
export interface Session { token: string; user: SessionUser }
function readSession(): Session | null {
  try {
    const token = localStorage.getItem("token");
    const user = JSON.parse(localStorage.getItem("user") || "null");
    return token && user && typeof user.id === "string" && typeof user.email === "string"
      ? { token, user: { ...user, currency: user.currency || "NGN" } } : null;
  } catch { return null; }
}
...
window.addEventListener("storage", event => {
  if (event.key === "token" || event.key === "user" || event.key === null) {
    ...
    session = readSession(); listeners.forEach(listener => listener());
  }
});
