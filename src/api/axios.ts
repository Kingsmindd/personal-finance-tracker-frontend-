...
function refreshSession(generation: number): Promise<string> {
  ...
  const promise = api
    ...
    .then((response) => {
      ...
      if (generation !== getSessionGeneration() || !current) {
        throw new axios.CanceledError("Session changed");
      }
...
api.interceptors.response.use(
  (response) => {
    ...
    if (config._generation !== getSessionGeneration()) {
      throw new axios.CanceledError("Session changed");
    }
  ...
  async (error) => {
    ...
    if (!current || generation !== getSessionGeneration()) {
      throw error;
    }

    original._retry = true;

    if (original.headers.Authorization === "Bearer " + current.token) {
